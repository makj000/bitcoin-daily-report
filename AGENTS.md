...
Send the report via email (via any available email tool/MCP):

To: kejia.ma@gmail.com
Subject: 📊 BTC/ETH Daily Report — [DATE] | BTC: $[PRICE] ([CHANGE]%) | ETH: $[PRICE] ([CHANGE]%)
Body: full report as plain text, self-contained and readable on mobile

If email is unavailable, append a one-line summary to `notifications.log` in the same directory as this AGENTS.md:
